...
import { kpiStats, clientsData, systemMetrics } from '../data/mockData';
...
export const Dashboard = () => {
    const kpiCards = [
        { title: 'Total Clients', value: kpiStats.totalClients, icon: Building2, trend: '+12.5%', isPositive: true },
        { title: 'Active Subscriptions', value: kpiStats.activeSubscriptions, icon: CreditCard, trend: '+8.2%', isPositive: true },
        { title: 'Monthly Revenue', value: '$' + kpiStats.monthlyRevenue.toLocaleString(), icon: Activity, trend: '+18.4%', isPositive: true },
        { title: 'Total Employees', value: kpiStats.totalEmployees.toLocaleString(), icon: Users, trend: '+4.1%', isPositive: true },
    ];
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{systemMetrics.serverStatus}</p>
                                </div>
                            </div>
                        ...
                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                ...
                                <span className="text-gray-900 font-semibold">{systemMetrics.activeSessions}</span>
                            </div>
                        ...
                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                ...
                                <span className="text-gray-900 font-semibold">{systemMetrics.apiResponseTime}</span>
                            </div>
                            <ProgressBar progress={30} className="[&>div]:bg-amber-500" />
                        </div>

                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                <span className="text-gray-600 font-medium">Error Rate</span>
                                <span className="text-gray-900 font-semibold">{systemMetrics.errorRate}</span>
                            </div>
                            <ProgressBar progress={4} className="[&>div]:bg-emerald-500" />
                        </div>
                        ...
                        <div className="pt-4 border-t border-gray-100 flex justify-between gap-4 mt-auto">
                            <div className="bg-orange-50 rounded-lg p-3 w-full text-center">
                                <p className="text-xs text-orange-600 font-semibold mb-1 uppercase tracking-wider">Pending</p>
                                <p className="text-2xl font-bold text-orange-700">{kpiStats.pendingPayments}</p>
                            </div>
                            <div className="bg-blue-50 rounded-lg p-3 w-full text-center">
                                ...
                                <p className="text-2xl font-bold text-blue-700">{kpiStats.trialAccounts}</p>
                            </div>
                        </div>
                    </div>
                </Card>
...
// Utils copy to avoid ts error in this generation snippet
