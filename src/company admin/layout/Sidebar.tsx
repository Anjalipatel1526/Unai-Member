...
export function Sidebar({ collapsed, setCollapsed, mobileOpen, setMobileOpen }: SidebarProps) {
    ...
    return (
        <>
            ...
            {mobileOpen && (
                <div
                    className="fixed inset-0 z-40 bg-gray-900/50 backdrop-blur-sm sm:hidden transition-opacity duration-300"
            ...
            <aside
                className={cn(
                    "fixed left-0 top-0 z-40 h-screen border-r border-gray-100 bg-white transition-all duration-300 ease-in-out flex flex-col shadow-xl sm:shadow-none",
                    collapsed ? "w-20" : "w-64",
            ...
            >
                ...
                <div className="flex h-16 items-center justify-between px-4 border-b border-gray-100">
                    ...
                    <button
                        ...
                        className="rounded-lg p-1.5 text-gray-500 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-200 shrink-0 hidden sm:block"
                    >
                        {collapsed ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
                    </button>
                    <button
                        onClick={() => setMobileOpen(false)}
                        className="rounded-lg p-1.5 text-gray-500 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-200 shrink-0 sm:hidden"
                    >
                        <X size={20} />
                    </button>
                </div>
